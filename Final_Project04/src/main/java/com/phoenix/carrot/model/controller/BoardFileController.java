...
public class BoardFileController {
    ...
    public ResponseEntity<List<String>> fileList(@PathVariable("entireBoardSeq") int entireBoardSeq) throws Exception {
        ...
        try {
            entity = new ResponseEntity<List<String>>(biz.getAttach(entireBoardSeq), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
 ...
 // 단일 파일 데이터 삭제2 : 게시글 수정화면
    @Transactional
    @ResponseBody
    @RequestMapping(value = "/delete/{entireBoardSeq}", method = RequestMethod.POST)
    public ResponseEntity<String> boardMofifyRemoveFile(@PathVariable("entireBoardSeq") int entireBoardSeq, String fileName, HttpServletRequest request) throws Exception {
        // DB에서 파일명 제거
    	biz.deleteAttach(fileName);
        // 첨부파일 갯수 갱신
    	//biz.updateAttachCnt(entireBoardSeq);
        // 파일 삭제
        UploadFileUtils.removeFile(uploadPath, fileName);
        return new ResponseEntity<String>("DELETED", HttpStatus.OK);
    }
